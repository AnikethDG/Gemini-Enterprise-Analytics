/*
    Step 3: Analytics & Interpretation
*/

-- A. Cluster Interpretation & Naming
-- Discover the topic of each cluster by analyzing representative prompts with Gemini.
WITH ClusterSamples AS (
  SELECT
    cluster_id,
    ARRAY_AGG(prompt ORDER BY nearest_centroid_distance ASC LIMIT 10) AS samples,
    COUNT(*) AS prompt_count
  FROM
    `bnoriega-test-ge.ConversationLogs.classification`
  GROUP BY
    cluster_id
)
SELECT
  cluster_id,
  TRIM(JSON_VALUE(ml_generate_text_result, '$.candidates[0].content.parts[0].text')) AS cluster_name,
  samples AS representative_prompts,
  prompt_count
FROM
  ML.GENERATE_TEXT(
    MODEL `bnoriega-test-ge.ConversationLogs.generative_model`,
    (
      SELECT
        cluster_id,
        prompt_count,
        samples,
        CONCAT(
          'You are a data analyst. Analyze these user prompts to determine the common theme or topic. ',
          'Output ONLY a concise 2-5 word label for this cluster (e.g., "Creative Writing", "Code Debugging"). ',
          'Do not use any special characters or markup.\n\nPrompts:\n- ',
          ARRAY_TO_STRING(samples, '\n- ')
        ) AS prompt
      FROM
        ClusterSamples
    ),
    STRUCT(
      0.0 AS temperature,
      500 AS max_output_tokens
    )
  )
ORDER BY
  prompt_count DESC;


-- B. Outlier Detection (New Class Discovery)
-- Prompts that are very far from their assigned cluster center might represent NEW topics.
SELECT
  timestamp,
  prompt,
  cluster_id,
  nearest_centroid_distance
FROM
  `bnoriega-test-ge.ConversationLogs.classification`
ORDER BY
  nearest_centroid_distance DESC
LIMIT 100;


-- C. User Engagement by Topic
-- Which clusters are most active users engaging with?
SELECT
  cluster_id,
  COUNT(DISTINCT user_iam_principal) AS distinct_users,
  COUNT(*) AS total_prompts
FROM
  `bnoriega-test-ge.ConversationLogs.classification`
GROUP BY
  cluster_id
ORDER BY
  distinct_users DESC;


-- D. Topic Trends Over Time
-- How is the volume of each topic changing daily?
SELECT
  DATE(timestamp) AS date,
  cluster_id,
  COUNT(*) AS daily_volume
FROM
  `bnoriega-test-ge.ConversationLogs.classification`
GROUP BY
  1, 2
ORDER BY
  1 DESC, 3 DESC;
